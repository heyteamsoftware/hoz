...
(() => {
  ...
  function load() {
    ...
    loadP = fetch('/api/image-slots/load')
      .then((r) => (r.ok ? r.json() : {}))
      .then((j) => {
        // Merge: server loses to any in-memory change that raced ahead of
      ...
      .catch(() => {
        ...
        try {
          const backup = localStorage.getItem('image-slots-backup');
          if (backup) {
  ...
  function save() {
    ...
    fetch('/api/image-slots/save', {
      ...
      .catch(() => {
        ...
        try {
          localStorage.setItem('image-slots-backup', JSON.stringify(slots));
        } catch (e) {}
      })
  ...
  class ImageSlot extends HTMLElement {
    ...
    async connectedCallback() {
      // Hide until admin check resolves to avoid flash of upload UI for visitors
      this._empty.style.display = 'none';
      this._ring.style.display = 'none';
      this._frame.style.background = 'transparent';

      // Check if admin before enabling editing
      ...
      // Only enable drag-drop if admin
      if (this._isAdmin) {
    ...
    async _ingest(file) {
      ...
      if (!file || ACCEPT.indexOf(file.type) < 0) {
        this._setError('Solo PNG, JPEG, WebP o AVIF.');
        return;
      }
      const gen = ++this._gen;
      try {
        ...
        formData.append('slotId', this.id || 'unknown');
        ...
        if (!response.ok) {
          ...
          this._setError(err.error || 'Error al subir la imagen.');
          return;
        }
        ...
        if (gen !== this._gen) return;
        this._setError('No se pudo subir la imagen.');
    ...
    _render() {
      ...
      this.toggleAttribute('data-editable', this._isAdmin);
      // Ocultar toda la interfaz de carga para usuarios normales
      const hasPlaceholder = (this.getAttribute('placeholder') ?? '') !== '';
      this._empty.style.cursor = this._isAdmin ? 'pointer' : 'default';
      this._sub.style.display = (this._isAdmin && hasPlaceholder) ? '' : 'none';
      const emptyIcon = this._empty.querySelector('svg');
      if (emptyIcon) emptyIcon.style.display = (this._isAdmin && hasPlaceholder) ? '' : 'none';
      this._cap.style.display = (this._isAdmin && hasPlaceholder) ? '' : 'none';
      ...
      // Accept data URLs and server-hosted paths (/public/images/...)
      if (stored && stored.u && !/^data:image\//i.test(stored.u) && !/^\/public\//i.test(stored.u)) stored = null;
      ...
      if (url) {
        ...
        // Non-admin with no image: hide all upload UI completely
        this._empty.style.display = this._isAdmin ? 'flex' : 'none';
        this._ring.style.display = this._isAdmin ? '' : 'none';
        this._frame.style.background = this._isAdmin ? '' : 'transparent';
